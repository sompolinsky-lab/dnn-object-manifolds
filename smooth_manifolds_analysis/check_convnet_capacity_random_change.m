function check_convnet_capacity_random_change(P, range_factor, N_SAMPLES, network_type, degrees_of_freedom, local_preprocessing, input_suffix, run_id, random_labeling_type, layers_grouping_level, use_half_samples, features_type, N_OBJECTS)
if nargin < 6
...
function out_name = get_filenames(range_factor, N_OBJECTS, N_SAMPLES, network_name, random_labeling_type, local_preprocessing, degrees_of_freedom, input_suffix, use_half_samples, features_type, run_id)
...
if length(run_id) > 1
    run_ids = run_id;
    out_name = get_filenames(range_factor, N_OBJECTS, N_SAMPLES, network_name, random_labeling_type, local_preprocessing, degrees_of_freedom, input_suffix, use_half_samples, features_type, 0);
    fprintf('Results saved to %s\n', out_name);

    for run_id=run_ids
        run_name = get_filenames(range_factor, N_OBJECTS, N_SAMPLES, network_name, random_labeling_type, local_preprocessing, degrees_of_freedom, input_suffix, use_half_samples, features_type, run_id);
...
out_name = get_filenames(range_factor, N_OBJECTS, N_SAMPLES, network_name, random_labeling_type, local_preprocessing, degrees_of_freedom, input_suffix, use_half_samples, features_type, run_id);
fprintf('Results saved to %s\n', out_name);
    ...
    for l=RUN_LAYERS
        ...
        [capacity, separability, ~, radius, mean_half_width1, mean_argmax_norm1, ...
            ...
            check_binary_dichotomies_capacity(current_tuning_function, N_NEURON_SAMPLES, N_DICHOTOMIES, true, random_labeling_type, ...
                precision, max_samples, global_preprocessing, features_type, min(512, N), properties_type);
