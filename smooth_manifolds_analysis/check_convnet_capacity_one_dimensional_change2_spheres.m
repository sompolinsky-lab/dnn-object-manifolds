function check_convnet_capacity_one_dimensional_change2_spheres(P, range_factor, N_SAMPLES, network_type, round_dimension, ...
    run_id, epoch, D, center_norm_factor, features_type, N_OBJECTS, random_labeling_type, layers_grouping_level)
% Calculate capacity for spheres with the same geometry as the given manifolds.
%
...
function [out_name, common_corr_name] = get_filenames(range_factor, N_OBJECTS, N_SAMPLES, network_name, round_dimension, random_labeling_type, local_preprocessing, D, center_norm_factor, features_type, run_id)
    if bitand(round_dimension,3) == 1
        prefix = sprintf('check_%s_capacity_one_dimensional_change2_spheres_ceilD', network_name);
    elseif bitand(round_dimension,3) == 0
        prefix = sprintf('check_%s_capacity_one_dimensional_change2_spheres_floorD', network_name);
    elseif bitand(round_dimension,3) == 2
        prefix = sprintf('check_%s_capacity_one_dimensional_change2_spheres_roundD', network_name);
    end
    if bitand(round_dimension,4) == 4
        prefix = [prefix, '_ns'];
    end
...
used_dimensions_results     = nan(N_DIRECTIONS, N_LAYERS, N_OBJECTS);
used_radii_results          = nan(N_DIRECTIONS, N_LAYERS, N_OBJECTS);
theory_capacity_results     = nan(N_DIRECTIONS, N_LAYERS);

if length(run_id) > 1
    run_ids = run_id;
    out_name = get_filenames(range_factor, N_OBJECTS, N_SAMPLES, network_name, round_dimension, random_labeling_type, local_preprocessing, D, center_norm_factor, features_type, 0);
    fprintf('Results saved to %s\n', out_name);

    missing = 0;
    for run_id=run_ids
        run_name = get_filenames(range_factor, N_OBJECTS, N_SAMPLES, network_name, round_dimension, random_labeling_type, local_preprocessing, D, center_norm_factor, features_type, run_id);
        ...
        used_dimensions_results(param_id, l, :) = run_file.used_dimensions_results(param_id, l, :);
        used_radii_results(param_id, l, :) = run_file.used_radii_results(param_id, l, :);
    ...
    save(out_name, 'capacity_results', 'separability_results', 'neuron_samples_used_results', ...
        'used_dimensions_results', 'used_radii_results', 'theory_capacity_results', 'n_support_vectors_results', ...
...
[out_name, common_corr_name] = get_filenames(range_factor, N_OBJECTS, N_SAMPLES, network_name, round_dimension, random_labeling_type, local_preprocessing, D, center_norm_factor, features_type, run_id);
...
for param_id=RUN_DIRECTIONS
    for l=RUN_LAYERS
        ...
        Rm = squeeze(sqrt(mean(common_corr_file.mean_argmax_norm2_results(param_id, l, :, :), 3))); 
        Dm = squeeze(mean(common_corr_file.effective_dimension2_results(param_id, l, :, :), 3)); 
        Ac = common_corr_file.theory_capacity_results(param_id, l);
        ...
        original_centers = reshape(mean(current_tuning_function, 2), [N, N_OBJECTS]);
        if bitand(round_dimension,4) == 4
            ...
            current_tuning_function = current_tuning_function - common_components;
        end

        % Get centers and spheres from the data
        ...
        N_AXES = max(Ds);
        axes = nan(N, N_AXES, N_OBJECTS);
        minN = min(N, N_SAMPLES);
        assert(all(Ds <= minN), 'Cannot create enough axes due to low number of samples');
        for io=1:N_OBJECTS
            center = centers(:,io);
            cF = current_tuning_function(:,:,io) - center;
            [U, ~, ~] = svd(cF, 0);
            axes(:,1:Ds(io),io) = Rm(io)*norm(center)*U(:,1:Ds(io));
        end
    	theory_capacity_results(param_id, l) = Ac;
        used_dimensions_results(param_id, l, :) = Ds;
        used_radii_results(param_id, l, :) = Rm;

        [capacity, separability, ~, n_neuron_samples_used, n_support_vectors] = ...
            check_binary_dichotomies_capacity2_spheres(original_centers, axes, EXPECTED_PRECISION, 1, random_labeling_type, ...
                precision, max_samples, features_type);
        fprintf('Capacity: %1.3f (theory: %1.3f)\n', N_OBJECTS ./ capacity, Ac);
